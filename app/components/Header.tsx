    ...
    <header
      className={`fixed left-0 top-0 z-50 w-full transition-all duration-300 ${
        scrolled
          ...
          : "border-b border-white/10 bg-white/80 backdrop-blur"
      }`}
    >
      <div
      ...
      >
        ...
        <nav className="hidden items-center gap-10 font-semibold text-slate-800 lg:flex">
          {menus.map((menu) => (
            <a key={menu.name} href={menu.href} className="hover:text-cyan-500">
              {menu.name}
            </a>
          ))}
        </nav>
        ...
        <a
          href="#contact"
          className="hidden rounded-full bg-cyan-500 px-7 py-3 font-bold text-white hover:bg-cyan-600 lg:block"
        >
          무료견적
        </a>
      ...
      {open && (
        <div className="border-t border-slate-200 bg-white px-5 py-6 shadow-xl lg:hidden">
          <nav className="flex flex-col gap-5 text-lg font-bold">
            {menus.map((menu) => (
              <a key={menu.name} href={menu.href} onClick={closeMenu}>
                {menu.name}
              </a>
            ))}
          </nav>
